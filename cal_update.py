...
from secrets import club_id, gc_id_prefix, calendar_name
...
class CalendarEvent:
    ...
    def process_entry(self):
        try:
            event_exists = None
            event_exists = self.service.events().get(calendarId=self.calendar_id,
                                                     eventId=f"{gc_id_prefix}{self.strava_event.id}").execute()
        ...
        if event_exists and event_exists['status'] == 'cancelled':
            print(
                f'ID:{self.strava_event.id} - Deleted from calendar, recreating...')
            is_deleted = True
    ...
    def create_update_entry(self, is_deleted=False):
        try:
            ...
            self.est_duration = round(
                (self.strava_route.distance / 1000)/20 + 1)
            self.has_route = True
        ...
        try:
            # Time
            start_time = self.strava_event.upcoming_occurrences[0].split('Z')[
                0]
            start_time_obj = datetime.datetime.fromisoformat(start_time)
            end_time_obj = start_time_obj + \
                datetime.timedelta(hours=self.est_duration)
            ...
            if is_deleted:
                self.service.events().update(calendarId=self.calendar_id, body=event,
                                             eventId=f"{gc_id_prefix}{self.strava_event.id}").execute()
            else:
...
Strava Event - {url}
""".format(description=self.strava_event.description,
...
def main():
    ...
    try:
        ...
        calendar_list = service.calendarList().list(syncToken=None,
                                                    minAccessRole=None,
                                                    maxResults=None,
                                                    showDeleted=None,
                                                    showHidden=None,
